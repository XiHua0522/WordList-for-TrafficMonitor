...
void CBookmarkDlg::RefreshList()
{
    ...
    for (const auto& bookmark_pos : g_data.GetBookmark())
    {
        //书签位置的百分比
        wchar_t buff[32];
        if (g_data.GetTextContexts().empty())
            swprintf_s(buff, 16, L"0.00%%");
        else
...
BOOL CBookmarkDlg::PreTranslateMessage(MSG* pMsg)
{
    ...
    {
        CWnd* pw = GetFocus();
        if (pw != nullptr && pw->m_hWnd == m_list_ctrl.m_hWnd)
        {
            int cur = m_list_ctrl.GetSelectionMark();
            if (cur >= 0 && cur < m_list_ctrl.GetItemCount())
            {
                int pos = static_cast<int>(m_list_ctrl.GetItemData(cur));
                if (AfxMessageBox(L"确认删除选中的书签吗？", MB_YESNO | MB_ICONQUESTION) == IDYES)
                {
                    g_data.RemoveBookmark(pos);
                    RefreshList();
                }
                return TRUE; // handled
            }
...
// deleted: OnBnClickedDeleteBookmark removed (use context-menu delete instead)
