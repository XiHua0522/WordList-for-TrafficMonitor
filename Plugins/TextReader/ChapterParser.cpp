...
void CChapterParser::Parse()
{
    ...
    while (true)
    {
        ...
        if (index == std::wstring::npos)
            break;
        // 查找后续的 '课' 或 '課'
        size_t index1 = m_contents.find(L"课", index + 2);
        size_t index1b = m_contents.find(L"課", index + 2);
        if (index1 == std::wstring::npos || (index1b != std::wstring::npos && index1b < index1))
            index1 = index1b;
        if (index1 == std::wstring::npos)
            continue;
        if (index1 - index < 10)
        {
            size_t index2 = m_contents.find_first_of(L"\r\n", index1 + 1);
            if (index2 != std::wstring::npos && index2 - index1 < 60)
            {
                std::wstring title = m_contents.substr(index, index2 - index);
                // 避免重复插入同一位置
                if (m_chapter.find(static_cast<int>(index)) == m_chapter.end())
                    m_chapter[static_cast<int>(index)] = title;
                index = index2 + 1;
            }
        }
